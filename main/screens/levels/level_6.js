...
export default class Level_6 extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    var level6Unlocked = await AsyncStorage.getItem('Level_6_Unlocked');
    ...
    await AsyncStorage.setItem('level', 'Level_6');
  ...
  setNewUnlocked = async () => {
    this.setState({ isVisible: false });
    await AsyncStorage.setItem('Level_6_Unlocked', 'true');
  }

  render() {
    return (
      <LinearGradient colors={['black', '#192f6a']} style={{ flex:1, alignItems: 'center', justifyContent: 'center' }}>
        {this.state.loading ?
          ...
          <View style={{ flex: 1, justifyContent: 'center' }}>
            <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, width: width - 80, padding: 15, borderRadius: 4, alignItems: 'center', backgroundColor: 'whitesmoke' }}>
              <Text style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, color: 'black', fontSize: 16 }}>Your Job: Luxury Restorant Owner</Text>
            </View>
              ...
              <Overlay
                ...
                >
                  <View style={{ alignItems: 'center' }}>
                    ...
                    <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.22, shadowRadius: 2.22, elevation: 3, marginTop: 5, alignItems: 'center', padding: 10, width: (width) - 40, backgroundColor: 'white'}}>
                    <Text style={{ fontSize: 16, color: 'black' }}>Now you are on Level 6</Text>
                    </View>
                    <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.22, shadowRadius: 2.22, elevation: 3, marginTop: 5, alignItems: 'center', padding: 10, width: (width) - 40, backgroundColor: 'white'}}>
                    <Text style={{ fontSize: 16, color: 'black' }}>You are owner of the luxury restorant now.</Text>
                    </View>
