...
export default class Level_3 extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    var level3Unlocked = await AsyncStorage.getItem('Level_3_Unlocked');
    ...
    await AsyncStorage.setItem('level', 'Level_3');
  ...
  setNewUnlocked = async () => {
    this.setState({ isVisible: false });
    await AsyncStorage.setItem('Level_3_Unlocked', 'true');
  }

  render() {
    return (
      <LinearGradient colors={['pink', '#192f6a']} style={{ flex:1, alignItems: 'center', justifyContent: 'center' }}>
        {this.state.loading ?
          ...
          <View style={{ flex: 1, justifyContent: 'center' }}>
            <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, width: width - 80, padding: 15, borderRadius: 4, alignItems: 'center', backgroundColor: 'whitesmoke' }}>
              <Text style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, color: 'pink', fontSize: 20 }}>Your Job: Hot-Dog Seller</Text>
            </View>
            ...
            <TouchableOpacity onPress={() => { this.earnMoney(); }}>
              <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, width: width - 80, height: width - 80, borderRadius: (width - 80) / 2, marginTop: 45, alignItems: 'center', justifyContent: 'center', backgroundColor: 'whitesmoke' }}>
                <Text style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, color: '#DB4437', fontSize: 40 }}>Sell Hot-Dog</Text>
              </View>
            </TouchableOpacity>
            {
              ...
              <Overlay
                ...
                >
                  <View style={{ alignItems: 'center' }}>
                    ...
                    <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.22, shadowRadius: 2.22, elevation: 3, marginTop: 5, alignItems: 'center', padding: 10, width: (width) - 40, backgroundColor: 'white'}}>
                    <Text style={{ fontSize: 16, color: 'black' }}>Now you are on Level 3</Text>
                    </View>
                    <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.22, shadowRadius: 2.22, elevation: 3, marginTop: 5, alignItems: 'center', padding: 10, width: (width) - 40, backgroundColor: 'white'}}>
                    <Text style={{ fontSize: 16, color: 'black' }}>You are hot-dog seller now. Let's sell HOT-Dog!</Text>
                    </View>
