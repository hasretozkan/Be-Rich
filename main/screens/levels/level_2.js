...
export default class Level_2 extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    var level2Unlocked = await AsyncStorage.getItem('Level_2_Unlocked');
    ...
    await AsyncStorage.setItem('level', 'Level_2');
  ...
  setNewUnlocked = async () => {
    this.setState({ isVisible: false });
    await AsyncStorage.setItem('Level_2_Unlocked', 'true');
  }

  render() {
    return (
      <LinearGradient colors={['#ffa457', '#192f6a']} style={{ flex:1, alignItems: 'center', justifyContent: 'center' }}>
        {this.state.loading ?
          ...
          <View style={{ flex: 1, justifyContent: 'center' }}>
            <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, width: width - 80, padding: 15, borderRadius: 4, alignItems: 'center', backgroundColor: 'whitesmoke' }}>
              <Text style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, color: '#ffa457', fontSize: 20 }}>Your Job: Water Seller</Text>
            </View>
            ...
            <TouchableOpacity onPress={() => { this.earnMoney(); }}>
              <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, width: width - 80, height: width - 80, borderRadius: (width - 80) / 2, marginTop: 45, alignItems: 'center', justifyContent: 'center', backgroundColor: 'whitesmoke' }}>
                <Text style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, color: '#DB4437', fontSize: 48 }}>Sell Water</Text>
              </View>
            </TouchableOpacity>
            {
              ...
              <Overlay
                ...
                >
                  <View style={{ alignItems: 'center' }}>
                    ...
                    <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.22, shadowRadius: 2.22, elevation: 3, marginTop: 5, alignItems: 'center', padding: 10, width: (width) - 40, backgroundColor: 'white'}}>
                    <Text style={{ fontSize: 16, color: 'black' }}>Now you are on Level 2</Text>
                    </View>
                    <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.22, shadowRadius: 2.22, elevation: 3, marginTop: 5, alignItems: 'center', padding: 10, width: (width) - 40, backgroundColor: 'white'}}>
                    <Text style={{ fontSize: 16, color: 'black' }}>You are water seller now. Let's sell water!</Text>
                    </View>
                    <TouchableOpacity onPress={() => { this.setNewUnlocked(); }}>
                      <View style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.22, shadowRadius: 2.22, elevation: 3, width: width - 40, backgroundColor: '#ffa457', padding: 10, justifyContent: 'center', alignItems: 'center', marginTop: 10 }}>
